...
WikidataSearch.prototype = {
     ...
     * occurred.
     */
    getEntities: function(entities, resolveProperties, requiredProps, cb) {
        
        requiredProps = requiredProps || ['description','label','claims'];
        ...
        request(requestOptions, function (error, response, wikidataResponse) {
            ...
            if (response === undefined) {
                ...
                for (var i in result.entities) {
                    ...
                    var valid = !(
                        (requiredProps.indexOf('description')!==-1 && !description)
                        || (requiredProps.indexOf('label')!==-1 && !label)
                        || (requiredProps.indexOf('claims')!==-1 && !claims.length)
                    );

                    if (valid){
                        var entityObj = {'id': id};
                        if (label)          entityObj.label = label;
                        if (description)    entityObj.description = description;
                        if (claims.length)  entityObj.claims = claims;
